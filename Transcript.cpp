...
void Transcript::printTranscript(const std::map<std::string, Module>& modules, int filterTerm) const {
    ...
    std::map<int, std::vector<Grade>> termGroups;
    ...
    for (const auto& termEntry : termGroups) {
        ...
        for (const Grade& grade : gradeList) {
            ...
            if (moduleRecord != modules.end()) {
                ...
                // Print module line
                std::cout << "    " << module.code() << " " << module.name() << " (" << module.credits() << " credits): " << grade.mark << "\n";

                // Accumulate marks and weighting for averages
                termWeightedScore += static_cast<double>(grade.mark) * module.credits();
        ...
        if (termCredits > 0) {
            ...
            std::cout << "  Term Average: " << std::fixed << std::setprecision(2) << termAvg << "\n";
    ...
    if (filterTerm == 0 && totalCredits > 0) {
        ...
        std::cout << "  Overall Average: " << std::fixed << std::setprecision(2) << overallAvg << "\n";
    }

    // New line for transcript format
    std::cout << "\n";
}